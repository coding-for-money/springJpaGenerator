...
import com.richcoder.code.generate.db.code.vo.ColumnInfo;
import com.richcoder.code.generate.db.code.vo.TableInfo;
import com.richcoder.code.generate.util.DBUtil;
import com.richcoder.code.generate.util.StringUtil;
import com.richcoder.code.generate.util.TypeUtil;
...
public class LoadDBInfo {
	...
	public static List<TableInfo> queryTableInfos(Set<String> skipTables, Set<String> initTables, Set<String> skipTablePrefix) {
		ResultSet rs = null;
		...
		try {
			...
			while (rs.next()) {
				...
				TableInfo tableMeta = new TableInfo(tableName, comment, className, primaryKey, columnInfos, columnTypes);
				list.add(tableMeta);
			}
		} catch (SQLException e) {
			e.printStackTrace();
	...
	//				TODO 将类型转化 移到工具类
				...
				ColumnInfo columnInfo = new ColumnInfo(colName, colClassName, attrName, attrType, columnComment);
				list.add(columnInfo);
			}
		}finally{
			colRet.close();
